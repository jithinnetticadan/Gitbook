...
* `EDITF_ATTRIBUTESUBJECTALTNAME2` is a **CA-wide** policy flag (not per-template). When set, the CA honors a custom SAN supplied in the request's *attributes* (`-attrib "SAN:..."`), regardless of whether the specific template has `ENROLLEE_SUPPLIES_SUBJECT` set.
